...
def test_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, task_dict in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if isinstance(task_dict['target_labels'], dict):
                ...
                preds = target_logits
                num_topks_correct = metrics.topks_correct(preds, task_dict['target_labels'], (1, 5))#统计top1和top5的正确个数
    ...
    for perclass in top1_per_class:
        ...
        top1_acc = top1_per_class[perclass]/num_per_class[perclass]
        totol_acc += top1_acc
        logger.info("class: {}, acc: {}".format(perclass, top1_acc))
        # print("class: {}, acc: {}".format(perclass, top1_acc))
    logger.info("totol_acc: {}".format(totol_acc/len(top1_per_class)))
    # print("totol_acc: {}".format(totol_acc/len(top1_per_class)))
    # write to tensorboard format if available.
    if writer is not None:
        if cfg.DETECTION.ENABLE:
            writer.add_scalars(
