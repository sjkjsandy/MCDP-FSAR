...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        x = prompts + self.positional_embedding.type(self.dtype)    #prompts : torch.Size([50, 77, 512])    positional_embedding : torch.Size([77, 512])
        x = x.permute(1, 0, 2)  # NLD -> LND
        with torch.no_grad():
            x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype) #正则化，规范化
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        x = x[torch.arange(x.shape[0]), tokenized_prompts.argmax(dim=-1)] @ self.text_projection   #x.shape = [batch_size, 512]

        return x  #torch.Size([50, 512])

class CrossModalAttentionPromptGenerator(nn.Module):
    """基于交叉注意力机制的提示词生成器"""
    def __init__(
        self,
        visual_dim: int = 512,
    ...
    def forward(
        ...
        attended = self.norm1(attended.squeeze(1) + text_proj.squeeze(1))
...
class MultimodalContextualPromptLearner(nn.Module):
    def __init__(self, cfg, clip_model):
        super().__init__()
        txt_path = cfg.DATA.ANNO_DIR
        class_real_train = cfg.TRAIN.CLASS_NAME
        class_real_test = cfg.TEST.CLASS_NAME
        self.n_cls_train = len(class_real_train)
        self.n_cls_test = len(class_real_test)
        n_ctx = 16  
        if hasattr(cfg.TRAIN, "N_CTX") and cfg.TRAIN.N_CTX:
            n_ctx = cfg.TRAIN.N_CTX
        ctx_init =""  
        if hasattr(cfg.TRAIN, "CTX_INIT") and cfg.TRAIN.CTX_INIT:
            ctx_init = cfg.TRAIN.CTX_INIT
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]  #512
        vis_dim = clip_model.visual.output_dim  #rn50: 1024, vit: 512
        ...
        if ctx_init:
            ...
            prompt = tokenize(ctx_init).cuda()
            with torch.no_grad():
                embedding = clip_model.token_embedding(prompt).type(dtype)
            ctx_vectors = embedding[0, 1 : 1 + n_ctx, :]
            prompt_prefix = ctx_init
        else:
            # random initialization 随机初始化上下文向量
            ctx_vectors = torch.empty(n_ctx, ctx_dim, dtype=dtype)  # (n_ctx, ctx_dim) torch.Size([16, 512])
        ...
        # Prompt generation with cross-modal self-attention
        self.crossmodal_promptgenerator = CrossModalAttentionPromptGenerator(
            visual_dim=vis_dim,
            text_dim=ctx_dim
        )
        
        #--------------------构建训练集的前缀和后缀--------------
        class_real_train = [name.replace("_", " ") for name in class_real_train]  #'crocodile_head' -> 'crocodile head'
        # name_lens = [len(_tokenizer.encode(name)) for name in class_real_train] # crocodile->[24757]，crocodile head->[24757, 1375]
        prompts = [prompt_prefix + " " + name + "." for name in class_real_train]  
        self.tokenized_prompts_train = torch.cat([tokenize(p) for p in prompts])  # (n_cls, n_tkn)  torch.Size([50, 77]),  'crocodile head' -> tensor([[ 4096,24757,1375,4097,0,0,0,0...]])
        prompts_no_class = [prompt_prefix + "." for _ in class_real_train]  
        self.tokenized_prompts_train_no_class = torch.cat([tokenize(p) for p in prompts_no_class])  
        desc = []
        if os.path.exists(txt_path):
            try:
                ...
                print(f"读取文件时出错: {e}")
        self.desc = torch.cat([tokenize(p) for p in desc])
        with torch.no_grad():
            embedding = clip_model.token_embedding(self.tokenized_prompts_train.cuda()).type(dtype)   #torch.Size([50, 77, 512])
            embedding_no_class = clip_model.token_embedding(self.tokenized_prompts_train_no_class.cuda()).type(dtype)   #torch.Size([50, 77, 512])
            self.desc = clip_model.token_embedding(self.desc.cuda()).type(dtype)
        # These token vectors will be saved when in save_model(),
        # but they should be ignored in load_model() as we want to use
        # those computed using the current class names
        self.register_buffer("token_prefix_train", embedding[:, :1, :])  # SOS
        self.register_buffer("token_suffix_train", embedding[:, 1 + n_ctx :, :])  # CLS, EOS
        self.register_buffer("token_suffix_train_no_class", embedding_no_class[:, 1 + n_ctx :, :])  # CLS, EOS
        #--------------------构建测试集的前缀和后缀--------------
        class_real_test = [name.replace("_", " ") for name in class_real_test]  #'crocodile_head' -> 'crocodile head'
        # name_lens = [len(_tokenizer.encode(name)) for name in class_real_test] 
        prompts = [prompt_prefix + " " + name + "." for name in class_real_test]  
        self.tokenized_prompts_test = torch.cat([tokenize(p) for p in prompts]) 
        prompts_no_class = [prompt_prefix + "." for _ in class_real_test]  
        self.tokenized_prompts_test_no_class = torch.cat([tokenize(p) for p in prompts_no_class])
        with torch.no_grad():
            embedding = clip_model.token_embedding(self.tokenized_prompts_test.cuda()).type(dtype)   #torch.Size([50, 77, 512])
            embedding_no_class = clip_model.token_embedding(self.tokenized_prompts_test_no_class.cuda()).type(dtype)   #torch.Size([50, 77, 512])
        # These token vectors will be saved when in save_model(),
        # but they should be ignored in load_model() as we want to use
        # those computed using the current class names
        self.register_buffer("token_prefix_test", embedding[:, :1, :])  # SOS ,embedding: torch.Size([21, 77, 512])
        self.register_buffer("token_suffix_test", embedding[:, 1 + n_ctx :, :])  # CLS, EOS
        self.register_buffer("token_suffix_test_no_class", embedding_no_class[:, 1 + n_ctx :, :])  # CLS, EOS
    ...
    def forward(self, im_features, txt_features, isTrain = True, isNoClass = True):
        prefix = self.token_prefix_train if isTrain else self.token_prefix_test  #torch.Size([50, 1, 512])  
        if isNoClass:
            suffix = self.token_suffix_train_no_class if isTrain else self.token_suffix_test_no_class  
        else:
            suffix = self.token_suffix_train if isTrain else self.token_suffix_test  
        ctx = self.ctx                     # (n_ctx, ctx_dim)   torch.Size([8, 512])
        bias = self.crossmodal_promptgenerator(im_features, txt_features) 
        bias = bias.unsqueeze(1)           # (batch, 1, ctx_dim)  torch.Size([1, 1, 512])
        ctx = ctx.unsqueeze(0)             # (1, n_ctx, ctx_dim) torch.Size([1, 8, 512])
        ctx_shifted = ctx + bias           # (batch, n_ctx, ctx_dim)  torch.Size([1, 8, 512])
        # Use instance-conditioned context tokens for all classes
        prompts = []
        for ctx_shifted_i in ctx_shifted:  #每一个ctx_shifted_i:torch.Size([8, 512]) 
            ctx_i = ctx_shifted_i.unsqueeze(0).expand(self.n_cls_train if isTrain else self.n_cls_test, -1, -1)  # ctx_i  torch.Size([50, 8, 512])
            pts_i = self.construct_prompts(ctx_i, prefix, suffix)  # (n_cls, n_tkn, ctx_dim), pts_i  torch.Size([50, 77, 512])
            prompts.append(pts_i)
        prompts = torch.stack(prompts)  # (batch, n_cls, n_tkn, ctx_dim)  torch.Size([1, 50, 77, 512])
...
class CNN_OTAM_CLIPFSAR(CNN_FSHead):
    ...
    def __init__(self, cfg):
        ...
        if cfg.VIDEO.HEAD.BACKBONE_NAME=="RN50":
            ...
            self.multimodal_contextual_prompt_learner = MultimodalContextualPromptLearner(cfg, backbone)
            self.tokenized_prompts_train = self.multimodal_contextual_prompt_learner.tokenized_prompts_train
            self.tokenized_prompts_test = self.multimodal_contextual_prompt_learner.tokenized_prompts_test
        ...
        with torch.no_grad():
            ...
            self.text_features_train = backbone.encode_text(text_templete) #torch.Size([70, 1024]),提示词特征  维度1024， 70个训练的类别数
            ...
            text_templete = tokenize(text_templete).cuda() #torch.Size([21, 77]),  提示词编码
            self.text_features_test = backbone.encode_text(text_templete)  #torch.Size([21, 1024]) # 提示词特征  维度1024，21个测试的类别数

            self.index_frame_to_txtfeature = 3
            if hasattr(self.args.DATA, "INDEX_FRAME_TO_TXTFETURE") and self.args.DATA.INDEX_FRAME_TO_TXTFEATURE:
                self.index_frame_to_txtfeature = self.args.DATA.INDEX_FRAME_TO_TXTFEATURE
    ...
    def get_feats(self, support_images, target_images):
        ...
        if self.training:
            ...
            support_features = self.backbone(support_images).squeeze()#获取每个支持视频的关键帧的图像特征 torch.Size([40, 1024])
            target_features = self.backbone(target_images).squeeze()#获取每个支持视频的关键帧的图像特征 torch.Size([40, 1024])
            dim = int(target_features.shape[1])
            target_features = target_features.reshape(-1, self.args.DATA.NUM_INPUT_FRAMES, dim) #torch.Size([5, 8, 1024])
            support_features = support_features.reshape(-1, self.args.DATA.NUM_INPUT_FRAMES, dim) #torch.Size([5, 8, 1024])

        return support_features, target_features
    
    def get_text_features(self, image_features, text_features_desc = None, isTrain = True, isNoClass = True):
        #用image_features 作为输入，计算text_features，用图像特征 计算提示词的向量
        prompts = self.multimodal_contextual_prompt_learner(image_features, text_features_desc, isTrain, isNoClass) #torch.Size([1, 50, 77, 512])    (batch, n_cls, n_tkn, ctx_dim)
        
        text_features_all = []
        for pts_i in prompts: #pts_i  torch.Size([50, 77, 512])
            text_features = self.text_encoder(pts_i, self.tokenized_prompts_train if isTrain else self.tokenized_prompts_test)  # (n_cls, ctx_dim)  text_features  torch.Size([50, 512])
            # text_features = text_features / text_features.norm(dim=-1, keepdim=True)
    ...
    def forward(self, inputs):
        support_images, support_labels, target_images, support_real_class, target_real_class, target_vid,support_vid = inputs['support_set'], inputs['support_labels'], inputs['target_set'], inputs['real_support_labels'], inputs['real_target_labels'], inputs['target_vid'], inputs['support_vid']
        support_features_text = self.multimodal_contextual_prompt_learner.desc[support_vid]
        target_features_text = self.multimodal_contextual_prompt_learner.desc[target_vid]
        # set_trace()
        if self.training:
            support_features, target_features = self.get_feats(support_images, target_images)
            ...
            if self.training:
                image_features = support_features[:, self.index_frame_to_txtfeature, :]  #support_features torch.Size([5, 8, 1024]) --> image_features torch.Size([5, 1024])
                image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                context_support = self.get_text_features(image_features, support_features_text, isTrain=True, isNoClass=True)
                c = torch.zeros((image_features.size(0), 1, image_features.size(1))).cuda()  # torch.Size([5
                # 使用索引从a中提取数据到c
                for i in range(c.size(0)):  # 遍历c的第一维
                    # index = self.class_real_train_index[support_real_class[i].long()]  # 根据b的元素进行索引
                    c[i, 0, :] = context_support[i, support_real_class[i].long(), :]
                context_support = c

                image_features = target_features[:, self.index_frame_to_txtfeature, :]  #target_features torch.Size([5, 8, 1024]) --> image_features torch.Size([5, 1024])
                image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                context_support_target = self.get_text_features(image_features, target_features_text, isTrain=True, isNoClass=True)
                c = torch.zeros((image_features.size(0), 1, image_features.size(1))).cuda()  # torch.Size([5
                # 使用索引从a中提取数据到c
                for i in range(c.size(0)):  # 遍历c的第一维
                    # index = self.class_real_test_index[target_real_class[i].long()]  # 根据b的元素进行索引
                    c[i, 0, :] = context_support_target[i, target_real_class[i].long(), :]
                context_support_target = c
                # context_support = self.text_features_train[support_real_class.long()].unsqueeze(1)#.repeat(1, self.args.DATA.NUM_INPUT_FRAMES, 1)
            
            else:
                image_features = support_features[:, self.index_frame_to_txtfeature, :]  #support_features torch.Size([5, 8, 1024]) --> image_features torch.Size([5, 1024])
                image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                context_support = self.get_text_features(image_features, support_features_text, isTrain=False, isNoClass=True)
                c = torch.zeros((image_features.size(0), 1, image_features.size(1))).cuda()  # torch.Size([5
                # 使用索引从a中提取数据到c
                for i in range(c.size(0)):  # 遍历c的第一维
                    # index = self.class_real_test_index[support_real_class[i].long()]  # 根据b的元素进行索引
                    c[i, 0, :] = context_support[i, support_real_class[i].long(), :]
                context_support = c
                #--------------------
                image_features = target_features[:, self.index_frame_to_txtfeature, :]  #target_features torch.Size([5, 8, 1024]) --> image_features torch.Size([5, 1024])
                image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                context_support_target = self.get_text_features(image_features, target_features_text, isTrain=False, isNoClass=True)
                c = torch.zeros((image_features.size(0), 1, image_features.size(1))).cuda()  # torch.Size([5
                # 使用索引从a中提取数据到c
                for i in range(c.size(0)):  # 遍历c的第一维
                    # index = self.class_real_test_index[target_real_class[i].long()]  # 根据b的元素进行索引
            ...
            target_features = torch.cat([target_features, context_support_target], dim=1)
            target_features = self.context2(target_features, target_features, target_features)[:,:self.args.DATA.NUM_INPUT_FRAMES,:]
            ...
            if hasattr(self.args.TRAIN, "EVAL_TEXT") and self.args.TRAIN.EVAL_TEXT:
                ...
                support_features, target_features = self.get_feats(support_images, target_images)# [5,8,1024] 每个batch5个视频，每个视频8帧，每个帧1024维度的特征
                #在第一个维度合并support_features,target_features
                feature_classification_in = torch.cat([support_features,target_features], dim=0)  # [10,8,1024]
                feature_classification = self.classification_layer(feature_classification_in).mean(1) # [10,1024]
                class_text_logits = cos_sim(feature_classification, self.text_features_train)*self.scale
                #与训练集的文本特征self.text_features_train计算cosine相似度，得到logits
                
                if self.training:
                    image_features = support_features[:, self.index_frame_to_txtfeature, :]  #support_features torch.Size([5, 8, 1024]) --> image_features torch.Size([5, 1024])
                    image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                    context_support = self.get_text_features(image_features, support_features_text, isTrain=True, isNoClass=False)
                    c = torch.zeros((image_features.size(0), 1, image_features.size(1))).cuda()  # torch.Size([5
                    # 使用索引从a中提取数据到c
                    for i in range(c.size(0)):  # 遍历c的第一维
                        # index = self.class_real_train_index[support_real_class[i].long()]  # 根据b的元素进行索引
                    ...
                    context_support_target = c
                    # context_support = self.text_features_test[support_real_class.long()].unsqueeze(1)#.repeat(1, self.args.DATA.NUM_INPUT_FRAMES, 1) # .repeat(support_bs+target_bs, 1, 1)
                #这一步就是调用所谓原型调制模型，将查询视频特征调用原型调制  target_real_class
                # target_features = self.context2(target_features, target_features, target_features)
                target_features = torch.cat([target_features, context_support_target], dim=1)
                target_features = self.context2(target_features, target_features, target_features)[:,:self.args.DATA.NUM_INPUT_FRAMES,:]
                #合并前，对每一个support_labels类别的support_features 做一个torch.mean均值计算
                if hasattr(self.args.TRAIN, "MERGE_BEFORE") and self.args.TRAIN.MERGE_BEFORE:
                    unique_labels = torch.unique(support_labels)
                    support_features = [torch.mean(torch.index_select(support_features, 0, extract_class_indices(support_labels, c)), dim=0) for c in unique_labels]
                    support_features = torch.stack(support_features)
                    ...
                    context_support = torch.stack(context_support)
                support_features = torch.cat([support_features, context_support], dim=1)
                #联合support_features, context_support后，做一个self.context2的attn操作，得到torch.Size([5, 8, 1024])的特征support_features
                #这一步就是调用所谓原型调制模型
                support_features = self.context2(support_features, support_features, support_features)[:,:self.args.DATA.NUM_INPUT_FRAMES,:]
                if hasattr(self.args.TRAIN, "MERGE_BEFORE") and self.args.TRAIN.MERGE_BEFORE:
                    pass
                ...
                target_features = rearrange(target_features, 'b s d -> (b s) d')    # [40, 1024]
                #计算每个查询视频关键帧与支持视频关键帧的相似度，得到torch.Size([40, 40])的相似度矩阵frame_sim
                frame_sim = cos_sim(target_features, support_features)    # [40, 40]
                frame_dists = 1 - frame_sim
                
                dists = rearrange(frame_dists, '(tb ts) (sb ss) -> tb sb ts ss', tb = n_queries, sb = n_support)  # [5, 5, 8, 8]

                # calculate query -> support and support -> query
                if hasattr(self.args.TRAIN, "SINGLE_DIRECT") and self.args.TRAIN.SINGLE_DIRECT:
                    cum_dists = OTAM_cum_dist_v2(dists)
                else:#通过OTAMję计算每个查询视频与支持视频的距离，得到torch.Size([5, 5])的距离矩阵cum_dists
                    cum_dists = OTAM_cum_dist_v2(dists) + OTAM_cum_dist_v2(rearrange(dists, 'tb sb ts ss -> tb sb ss ts')) # [5, 5]
...
@HEAD_REGISTRY.register()
class CNN_OTAM_CLIPFSAR_V0(CNN_FSHead):
    """
    OTAM with a CNN backbone.
    """
    def __init__(self, cfg):
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            if hasattr(self.args.TRAIN, "EVAL_TEXT") and self.args.TRAIN.EVAL_TEXT:
                ...
                support_features, target_features, _ = self.get_feats(support_images, target_images, support_labels)# [5,8,1024] 每个batch5个视频，每个视频8帧，每个帧1024维度的特征
                support_bs = support_features.shape[0]
                target_bs = target_features.shape[0]
                #在第一个维度合并support_features,target_features
                feature_classification_in = torch.cat([support_features,target_features], dim=0)  # [10,8,1024]
