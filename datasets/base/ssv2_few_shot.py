...
class Ssv2_few_shot(BaseVideoDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.TRAIN.META_BATCH:
            ...
            #select classes to use for this task，UCF中有21个测试类别，及其对应的所有视频数 2745个
            c = self.split_few_shot
            #获取当前数据集中所有类别，不重复
            classes = c.get_unique_classes()
            #随机选择way个类别做为一个batch，5个类别做为一个batch
            ...
            if self.split == "train":
                n_queries = self.cfg.TRAIN.QUERY_PER_CLASS #每个类别的查询数
            ...
            for retry in range(retries):
                try:
                    ...
                    for bl, bc in enumerate(batch_classes):
                        ...
                        # for retry in range(retries):
                        #     try:
                        #随机选择shot+query个视频作为support set
            ...
            support_vid = torch.FloatTensor(support_vid)
            target_vid = torch.FloatTensor(target_vid)
            ...
            return {"support_vid":support_vid, "target_vid":target_vid, "support_set":support_set, "support_labels":support_labels, "target_set":target_set, "target_labels":target_labels, "real_target_labels":real_target_labels, "batch_class_list": batch_classes, "real_support_labels":real_support_labels}
    ...
    def get_seq(self, label, idx=-1):
        ...
        c = self.split_few_shot
        if self.cfg.TRAIN.META_BATCH:
            paths, vid_id = c.get_rand_vid(label, idx) #从数据集中获取类别为label的视频，idx为-1时随机选择一个视频，否则选择类别为label的第idx个视频
            ...
            if not success:
                logger.info("Error at decoding. Vid index: {}, Vid path: {}".format(
                    index, sample_info["path"]))
                return self.__getitem__(index - 1) if index != 0 else self.__getitem__(index + 1)
            if data["video"].numel() == 0:#获取张量 data["video"] 的元素总数
                logger.info("data[video].numel()=0. Vid index: {}, Vid path: {}".format(
                    index, sample_info["path"]))
                return self.__getitem__(index - 1) if index != 0 else self.__getitem__(index + 1)
            # if self.split in ["test"] and self.cfg.TEST.ZERO_SHOT:
            ...
            if self.cfg.PRETRAIN.ENABLE:
                ...
                if "flow" in data.keys() and "video" in data.keys():
                    ...
                    data["video"] = self.transform(data["video"]) # C, T, H, W = 3, 8, 224, 224, RGB
            ...
            return data["video"].permute(1,0,2,3), vid_id   # C, T, H, W = 3, 8, 224, 224  -- >  T, C, H, W = 8, 3, 224, 224
